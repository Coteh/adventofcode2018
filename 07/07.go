...
func calculateTimeRequired(step rune, fixedTimeAmount int) int {
	return fixedTimeAmount + (int(step) - int('A') + 1)
}

func workSteps(stepTable *StepTable, numWorkers int, fixedTimeAmount int, debug bool) int {
	time := 0
	...
	for !stepsCompleted {
		...
		for _, workItem := range workItems {
			if !workItem.isWorking {
				...
				workItem.endTime = time + calculateTimeRequired(workItem.stepNode.step, fixedTimeAmount)
...
func main() {
	...
	numWorkers := flag.Int("num-workers", 5, "Number of workers that will be working on the work items")
	fixedTimeAmount := flag.Int("fixed-time-amount", 60, "Amount of time each step will take minimum")
	flag.Parse()
	...
	pt2Answer := workSteps(stepTable, *numWorkers, *fixedTimeAmount, *debugFlag)
